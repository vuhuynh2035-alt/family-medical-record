...
const CURRENT_APP_VERSION = 'v2.9.75';
const APP_CHANGELOG = {
            ...
            if (!summaryHTML.includes('<br>') && r.recordIds && r.recordIds.length > 0) {
                ...
                if (usedRecords.length > 0) {
                    ...
                    summaryHTML = `Được đánh giá từ ${sortedRecords.length} hồ sơ:<br>` + 
                              sortedRecords.map(rec => `- ${rec.disease || rec.hospital || 'Khám bệnh'} (Ngày ${formatDateShort(rec.date)})`).join('<br>');
        ...
        try {
            ...
            if (records.length > 0) {
                ...
                summary = `Được đánh giá từ ${sortedRecords.length} hồ sơ:<br>` + 
                          sortedRecords.map(r => `- ${r.disease || r.hospital || 'Khám bệnh'} (Ngày ${formatDateShort(r.date)})`).join('<br>');
            }
...
document.getElementById('btn-tts-mode')?.addEventListener('click', () => {
    ...
    if (TTSService.isPlaying && !TTSService.isPaused) {
        ...
        // Hủy âm thanh mà KHÔNG gọi stop() để giữ thanh đen hiển thị
        ...
        // Giữ isPlaying = true để thanh đen không biến mất
        ...
        setTimeout(() => {
            if (TTSService.isPlaying) TTSService.playNextChunk();
        }, 200);
    }
});
