...
const CURRENT_APP_VERSION = 'v2.9.85';
const APP_CHANGELOG = {
...
function setupEventListeners() {
    ...
    function renderTrendReportsList() {
        ...
        reports.forEach(r => {
            let summaryHTML = r.comparedRecordsSummary || '';
            if (!summaryHTML.includes('<br>') && r.recordIds && r.recordIds.length > 0) {
                const allRecords = DataManager.getRecords(currentMemberId);
                const usedRecords = allRecords.filter(rec => r.recordIds.includes(rec.id));
                if (usedRecords.length > 0) {
                    const sortedRecords = [...usedRecords].sort((a,b) => new Date(a.date) - new Date(b.date));
                    summaryHTML = `Được đánh giá từ ${sortedRecords.length} hồ sơ:<br>` + 
                              sortedRecords.map(rec => { let name = rec.hospital || rec.disease || 'Khám bệnh'; if (name.length > 35) name = name.substring(0, 35) + '...'; return `- Hồ sơ: ${name} (Ngày ${formatDateShort(rec.date)})`; }).join('<br>');
                    r.comparedRecordsSummary = summaryHTML;
                    DataManager.saveTrendReport(currentMemberId, r);
                }
            }
            ...
            div.innerHTML = `
                <div style="display: flex; justify-content: space-between; align-items: flex-start;">
                    <div>
                        ...
                        <div style="font-size: 12px; color: var(--text-muted); margin-top: 2px;">${summaryHTML}</div>
                    </div>
        ...
        try {
            ...
            content.querySelectorAll('h2, h3').forEach(heading => {
                ...
                playBtn.addEventListener('click', (e) => {
                    e.stopPropagation();
                    if (TTSService && TTSService.isPlaying && TTSService.activeBtnElement === playBtn) {
                        TTSService.stop();
                        return;
                    }
            ...
            if (records.length > 0) {
                ...
                summary = `Được đánh giá từ ${sortedRecords.length} hồ sơ:<br>` + 
                          sortedRecords.map(r => { let name = r.hospital || r.disease || 'Khám bệnh'; if (name.length > 35) name = name.substring(0, 35) + '...'; return `- Hồ sơ: ${name} (Ngày ${formatDateShort(r.date)})`; }).join('<br>');
            }
        ...
        if (reportContainer) {
            ...
            reportContainer.querySelectorAll('.dose-cell').forEach(cell => {
                cell.addEventListener('click', function() {
                    ...
                    // Bỏ gọi window.openMedicationPlanModal(plan) để tránh reset UI
                });
            });
        }
...
const TTSService = {
    ...
    speak(text, title = 'Đang đọc nội dung...', btnTarget = null, type = null, containerTarget = null, startChunkText = null) {
        ...
        // Tự động yêu cầu giữ màn hình sáng khi bắt đầu đọc
    ...
    setContainerHighlight(isHighlight) {
        // Tắt hiệu ứng viền xanh theo yêu cầu người dùng
    },
...
// Wake Lock để giữ màn hình sáng trong khi đọc
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (viewRecordContent) {
        viewRecordContent.addEventListener('click', (e) => {
            ...
            if (TTSService && TTSService.isPlaying && TTSService.chunks.length > 0) {
            ...
            if (fullText && TTSService) {

                TTSService.speak(fullText, 'Đọc nội dung', null, 'karaoke', container, clickedText);
            }
        });
    }
});
